...
class QuickSettings {
    ...
    private final String QS_UPDATE_TORCH_TILE_INTENT = "QS_UPDATE_TORCH_TILE_INTENT";
    private final String QS_VOLUME_OBSERVER_STREAM_INTENT = "QS_VOLUME_OBSERVER_STREAM_INTENT";
    ...
    private List<String> getEnabledTiles() {
        ...
        String tilesListString = Settings.System.getString(mContext.getContentResolver(), QS_ENABLED_TILES);
        if (tilesListString != null) {
            tiles = Arrays.asList(tilesListString.split(";"));
        } else {
            tiles = Arrays.asList(QS_SETTINGS, QS_BATTERY, QS_BRIGHTNESS, QS_RINGER, QS_WIFI, QS_BLUETOOTH, QS_LOCATION);
        }
        return tiles;
    }
    ...
    private void addTile(String tile, ViewGroup parent, LayoutInflater inflater) {
        if (tile.equals(QS_SETTINGS)) {
        ...
        } else if (tile.equals(QS_BRIGHTNESS)) {
            // Brightness
        ...
        } else if (tile.equals(QS_MOBILE_DATA)) {
        ...
        } else if (tile.equals(QS_ROTATION)) {
            // Rotation Lock
        ...
        } else if (tile.equals(QS_BATTERY)) {
            // Battery
        ...
        } else if (tile.equals(QS_AIRPLANE)) {
            // Airplane Mode
        ...
        } else if (tile.equals(QS_BLUETOOTH)) {
            // Bluetooth
            ...
            mModel.addRingerTile(ringerTile, new QuickSettingsModel.RefreshCallback() {
                @Override
                public void refreshView(QuickSettingsTileView view, State state) {
                    TextView textView = (TextView) view.findViewById(R.id.ringer_textview);
                    ImageView imageView = (ImageView) view.findViewById(R.id.ringer_overlay_image);

                    AudioManager audioManager = (AudioManager) mContext
                                    .getSystemService(Context.AUDIO_SERVICE);
                    switch (audioManager.getRingerMode()) {
                        case AudioManager.RINGER_MODE_NORMAL:
                            textView.setText(R.string.quick_settings_ringer_normal_label);
                            imageView.setImageResource(R.drawable.stat_ring_on);
                            break;
                        case AudioManager.RINGER_MODE_VIBRATE:
                            textView.setText(R.string.quick_settings_ringer_vibrate_label);
                            imageView.setImageResource(R.drawable.stat_ring_vibrate);
                            break;
                        case AudioManager.RINGER_MODE_SILENT:
                            textView.setText(R.string.quick_settings_ringer_silent_label);
                            imageView.setImageResource(R.drawable.stat_ring_off);
                            break;
                    }
                }
            });
            parent.addView(ringerTile);
        } else if (tile.equals(QS_LOCATION)) {
            // Location
            ...
            mModel.addLocationTile(locationTile, new QuickSettingsModel.RefreshCallback() {
                ...
                public void refreshView(QuickSettingsTileView view, State state) {
                    ...
                    if (locationEnabled) {
                        textView.setText(R.string.quick_settings_location_on_label);
                        imageView.setImageResource(R.drawable.ic_qs_location_on);
                    } else {
                        textView.setText(R.string.quick_settings_location_off_label);
                        imageView.setImageResource(R.drawable.ic_qs_location_off);
                    }
                }
            });
            parent.addView(locationTile);
        } else if (tile.equals(QS_WIFI_AP) ) {
            // Wifi AP
            if (mModel.deviceHasMobileData()) {
                ...
                mModel.addWifiApTile(WifiApTile, new QuickSettingsModel.RefreshCallback() {
                    @Override
                    public void refreshView(QuickSettingsTileView view, State state) {
                        TextView textView = (TextView) view.findViewById(R.id.wifiap_textview);
                        ImageView imageView = (ImageView) view.findViewById(R.id.wifiap_image);
                        ...
                        if (WifiApState == WifiManager.WIFI_AP_STATE_DISABLED
                                || WifiApState == WifiManager.WIFI_AP_STATE_DISABLING) {
                            textView.setText(R.string.quick_settings_wifi_ap_off_label);
                            imageView.setImageResource(R.drawable.stat_wifi_ap_off);
                        } else if (WifiApState == WifiManager.WIFI_AP_STATE_ENABLED
                                || WifiApState == WifiManager.WIFI_AP_STATE_ENABLING) {
                            textView.setText(R.string.quick_settings_wifi_ap_on_label);
                            imageView.setImageResource(R.drawable.stat_wifi_ap_on);
                        }
                    }
                });
                parent.addView(WifiApTile);
            }
        } else if (tile.equals(QS_TORCH)) {
            // Torch
            ...
            torchTile.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    mContext.sendBroadcast(new Intent(QS_UPDATE_TORCH_TILE_INTENT));
                }
            });
            mModel.addTorchTile(torchTile, new QuickSettingsModel.RefreshCallback() {
                @Override
                public void refreshView(QuickSettingsTileView view, State state) {
                    TextView textView = (TextView) view.findViewById(R.id.torch_textview);
                    ImageView imageView = (ImageView) view.findViewById(R.id.torch_image);

                    if (mTorchEnabled) {
                        textView.setText(R.string.quick_settings_torch_on_label);
                        imageView.setImageResource(R.drawable.stat_flashlight_on);
                    } else {
                        textView.setText(R.string.quick_settings_torch_off_label);
                        imageView.setImageResource(R.drawable.stat_flashlight_off);
                    }
                }
            });
            parent.addView(torchTile);
        } else if (tile.equals(QS_SCREEN_OFF)) {
            // Screen
            final QuickSettingsBasicTile screenTile
                    = new QuickSettingsBasicTile(mContext);
            screenTile.setImageResource(R.drawable.ic_lock_power_off);
            ...
            mModel.addScreenTile(screenTile, new QuickSettingsModel.RefreshCallback() {
                ...
                public void refreshView(QuickSettingsTileView unused, State alarmState) {
                    screenTile.setText(mContext.getResources().getString(R.string.quick_settings_screen_off));
                }
            });
    ...
    private void addSliderTile(ViewGroup parent, LayoutInflater inflater) {
        ...
        seekbarTile.setColumnSpan(Settings.System.getInt(mContext.getContentResolver(), QS_COLUMNS, 4));

        final SeekBar volumeSeekbar = (SeekBar) seekbarTile.findViewById(R.id.volume_seekbar);
        final ImageView volumeImageView = (ImageView) seekbarTile.findViewById(R.id.sound_icon);
        final SeekBar brightnessSeekBar = (SeekBar) seekbarTile.findViewById(R.id.brightness_seekbar);
        final CheckBox brightnessCheckBox = (CheckBox) seekbarTile.findViewById(R.id.brightness_switch);

        brightnessCheckBox.setBackgroundDrawable(mContext.getResources().getDrawable(R.drawable.status_bar_toggle_button));

        final AudioManager audioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);
        final IPowerManager iPowerManager = IPowerManager.Stub.asInterface(ServiceManager.getService("power"));
        final PowerManager powerManager = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
        final int min = powerManager.getMinimumScreenBrightnessSetting();
        final int max = powerManager.getMaximumScreenBrightnessSetting();

        mModel.addSeekbarTile(seekbarTile, new QuickSettingsModel.RefreshCallback() {
            @Override
            public void refreshView(QuickSettingsTileView view, State state) {
                volumeSeekbar.setMax(audioManager.getStreamMaxVolume(mVolumeStream));
                volumeSeekbar.setProgress(audioManager.getStreamVolume(mVolumeStream));
                if (mVolumeStream == AudioManager.STREAM_MUSIC) {
                    volumeImageView.setImageResource(com.android.internal.R.drawable.ic_audio_vol);
                } else {
                    volumeImageView.setImageResource(com.android.internal.R.drawable.ic_audio_ring_notif);
                }
                ...
                brightnessSeekBar.setMax(max - min);
                brightnessSeekBar.setProgress(value - min);
            }
        });
        ...
        volumeImageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mContext.sendBroadcast(new Intent(QS_VOLUME_OBSERVER_STREAM_INTENT));
            }
        });
        ...
        volumeSeekbar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if (fromUser) {
                    audioManager.setStreamVolume(mVolumeStream, progress, 0);
                } else {
                    volumeSeekbar.setProgress(audioManager.getStreamVolume(mVolumeStream));
                }
            }
        ...
        brightnessSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                try {
                    iPowerManager.setTemporaryScreenBrightnessSettingOverride(progress + min);
                    Settings.System.putInt(mContext.getContentResolver(),
    ...
    private void addTemporaryTiles(final ViewGroup parent, final LayoutInflater inflater) {
        ...
        // Wifi Display
        if (isTileEnabled(QS_WIFI_DISPLAY)) {
    ...
    private void setTorchEnabled(boolean enabled) {
        ...
        if (enabled) {
            ...
            if (mWakeLock == null) {
                mWakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "QS_TORCH");
            }
            if (!mWakeLock.isHeld()) {
                mWakeLock.acquire();
            }
