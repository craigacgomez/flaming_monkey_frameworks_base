...
class QuickSettingsModel implements BluetoothStateChangeCallback,
    ...
    public QuickSettingsModel(Context context) {
        ...
        tileFilter.addAction(QS_UPDATE_TORCH_TILE_INTENT);
        tileFilter.addAction(Intent.ACTION_HEADSET_PLUG);
        tileFilter.addAction(QS_VOLUME_OBSERVER_STREAM_INTENT);
    ...
    void refreshSettingsTile() {
        if (!isTileEnabled(QS_SETTINGS)) return;
    ...
    void onAlarmChanged(Intent intent) {
        if (!isTileEnabled(QS_ALARM)) return;
    ...
    public void onAirplaneModeChanged(boolean enabled) {
        if (!isTileEnabled(QS_AIRPLANE)) return;
    ...
    public void onWifiSignalChanged(boolean enabled, int wifiSignalIconId,
        ...
        if (!isTileEnabled(QS_WIFI)) return;
        // TODO: If view is in awaiting state, disable
        Resources r = mContext.getResources();
    ...
    public void onMobileDataSignalChanged(
        ...
        if (!isTileEnabled(QS_MOBILE_DATA)) return;
    ...
    public void onBluetoothStateChange(BluetoothState bluetoothStateIn) {
        if (!isTileEnabled(QS_BLUETOOTH)) return;
        // TODO: If view is in awaiting state, disable
        Resources r = mContext.getResources();
    ...
    public void onBatteryLevelChanged(int level, boolean pluggedIn) {
        if (!isTileEnabled(QS_BATTERY)) return;
    ...
    public void onLocationGpsStateChanged(boolean inUse, String description) {
        if (!isTileEnabled(QS_LOCATION)) return;
    ...
    public void onWifiDisplayStateChanged(WifiDisplayStatus status) {
        if (!isTileEnabled(QS_WIFI_DISPLAY)) return;
    ...
    void onImeWindowStatusChanged(boolean visible) {
        if (!isTileEnabled(QS_IME)) return;
    ...
    void onRotationLockChanged() {
        if (!isTileEnabled(QS_ROTATION)) return;
    ...
    public void onBrightnessLevelChanged() {
        if (!isTileEnabled(QS_BRIGHTNESS)) return;
        Resources r = mContext.getResources();
    ...
    private class MobileDataObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            super.onChange(selfChange);
            if (isTileEnabled(QS_MOBILE_DATA)) {
                mRSSICallback.refreshView(mRSSITile, mRSSIState);
            }
        }
    }
    ...
    private final BroadcastReceiver tileReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(LocationManager.PROVIDERS_CHANGED_ACTION)) {
                if (isTileEnabled(QS_LOCATION)) {
                    mLocationCallback.refreshView(mLocationTile, mLocationState);
                }
            } else if (intent.getAction().equals(AudioManager.RINGER_MODE_CHANGED_ACTION)) {
                if (isTileEnabled(QS_RINGER)) {
                    mRingerCallback.refreshView(mRingerTile, mQsmState);
                }
            } else if (intent.getAction().equals(WifiManager.WIFI_AP_STATE_CHANGED_ACTION)) {
                if (isTileEnabled(QS_WIFI_AP)) {
                    mWifiApCallback.refreshView(mWifiApTile, mQsmState);
                }
            } else if (intent.getAction().equals(QS_UPDATE_TORCH_TILE_INTENT)) {
                if (isTileEnabled(QS_TORCH)) {
                    mTorchCallback.refreshView(mTorchTile, mQsmState);
                }
            } else if (intent.getAction().equals(Intent.ACTION_HEADSET_PLUG)
                    || intent.getAction().equals(QS_VOLUME_OBSERVER_STREAM_INTENT)) {
                if (isTileEnabled(QS_SEEKBAR)) {
                    mSeekbarCallback.refreshView(mSeekbarTile, mQsmState);
